...
class App extends Component {
  ...
  sceneSetup = () => {
    ...
    cameraYPosition.position.y =4.98;
  ...
  startAnimationLoop = () => {
    ...
    const cameraXRotation = this.scene.getObjectByName("cameraXRotation");
    if (cameraXRotation.rotation.x < 1) {
      cameraXRotation.rotation.x += 0.01;
    }

    const cameraZPosition = this.scene.getObjectByName("cameraZPosition");
    cameraZPosition.position.z -= 0.25;

    const cameraZRotation = this.scene.getObjectByName("cameraZRotation");
    cameraZRotation.rotation.z =
      this.noise.simplex2(timeElapsed * 1.5, timeElapsed * 1.5) * 0.02;

    const boxGrid = this.scene.getObjectByName("boxGrid");
    boxGrid.children.forEach((child, index) => {
      const x = timeElapsed * 5 + index + 1;
      child.scale.y = this.noise.simplex2(x, x) + 2 / 2;
      //child.position.y = child.scale.y-1 ;
    });
  };
